...
The tool we built is a real-time observability stack for distributed backend servers. It is not plug-and-play but it's customizable — you decide what to measure, what thresholds matter, what to alert on. But once configured, it gives you a complete picture of your system from the infrastructure layer down to what's happening inside each server process.

Like Yvone menytioned, it would be nice to build on top of other classmates' project. ChaosArea, is a excellent exmaple we can demo our tool, because we know very little what the load is actually like. So, we decided to mount our tool to chaos area server to do a demo. It allows us to see even the number of go routain is working in the sepaphore, so we can in theory know exactly what kind of load is being sent to us.
...
Kafka (event-driven) connects two consumers: an Analytics service that aggregates events into 60-second windows, and an Alert service that evaluates per-instance p95 latency every 30 seconds and fires SNS email alerts on threshold breaches.
Prometheus (pull-based) scrapes all endpoints every 15 seconds — the album store, both microservices, and exporters for Redis, PostgreSQL, CloudWatch, and S3.
...
locust -f loadtest/locustfile.py \
  ...
  --users 80 --spawn-rate 10 --run-time 20m --headless &
```
